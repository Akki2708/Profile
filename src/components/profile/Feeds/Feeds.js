...
//               onClick={(e) => {
//                 e.stopPropagation();
//                 handleLikeClick(feed.id);
//               }}
//             >
//               <FontAwesomeIcon
//                 icon={feed.isLiked ? faThumbsUpSolid : faThumbsUpRegular}
...
const Feeds = () => {
  ...
  // Repeat feeds multiple times
  const repeatedFeeds = [];
  for (let i = 0; i < 10; i++) {
    mockFeeds.forEach((feed) => {
      repeatedFeeds.push({
        ...feed,
        id: feed.id + i * mockFeeds.length, // unique ID
        time: feed.time,
      });
    });
  }

  // Use repeatedFeeds in state
  const [feeds, setFeeds] = useState(
    repeatedFeeds.map((feed) => ({ ...feed, isLiked: false }))
  );
  ...
  const handleCommentClick = (feedId) => {
    navigate(`/likes-comments/${feedId}`, { state: { overlay: true } });
  };
